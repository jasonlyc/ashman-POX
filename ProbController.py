...
class ProbController(EventMixin):
    ...
    def _demandEstimator(self):
        ...
        for flow in estFlows:
            ...
            if demand >= self.ratio:
                self._ProbabilisticFit(flow)

    def _ProbabilisticFit(self, flow):
        '''do probabilistic fit here'''
            ...
            for i in range(1, len(path)):
                ...
                if self.bwReservation.has_key(path[i - 1]) and self.bwReservation[path[i - 1]].has_key(path[i]):
                    if self.bwReservation[path[i - 1]][path[i]]['reserveDemand'] + flow['demand'] > 1:
                        ...
                        fitCheck = True
                        if 1 - self.bwReservation[path[i-1]][path[i]]['reserveDemand'] < residualCapacity:
                            residualCapacity = 1 - self.bwReservation[path[i-1]][path[i]]['reserveDemand']
                else:
                    ...
                    fitCheck = True
            if fitCheck == True:
                bwSum += 1 / residualCapacity
                pathProb.append(bwSum)
                availablePaths.append(path)
        dice = random.random()
        dice *= bwSum
        for i in range(0, len(pathProb)):
            if dice <= pathProb[i]:
                PF_route = availablePaths[i]
                break
        if PF_route != None:
            for i in range(1,len(PF_route)):
                self.bwReservation[PF_route[i-1]][PF_route[i]]['reserveDemand'] += flow['demand']
            """install new GBF_path between source and destintaion"""
            self._install_customized_path(PF_route,flow['match'])
...
def launch(topo = None, routing = None, bw = None, ratio = None ):
    ...
    if ratio == None:
        ratio = 0.1
    core.registerNew(ProbController, t, r, bw, ratio)
